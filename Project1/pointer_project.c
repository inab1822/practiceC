...
// 물고기가 6마리 살고 있다.
// 이들은 어항에 살고있는데 사막에 있다.
// 사막이 너무 더워서 너무 건조해서 물이 아주 빨리 증발한다.
// 물이다 증발하기 전에 부지런히 어항에 물을 줘서 물고리를 살려주세요.
...
int main(void)
{
	...
	int num; // 몇 번 어항에 물을 줄것인지.
	initData();

	cursor = arrayFish; // cursor[0]...cursor[1]...

	startTime = clock(); // 현재시각을 millisecond (1000분의1초) 단위로 반환
	while (1)
	{
		printFishes();
		...
		// 총 경과시간
		// CLOCKS_PER_SEC 는 밀리세컨드로 나오는 시간을 초 단위로 바꾸기위함이다.
		...
		// 사용자가 입력한 어항의 물을 준다.
		// 1. 어항의 물이 0이면? 물을 주지 않는다...이미 물고기가..죽어서...
		if (cursor[num-1]<=0)
		{	
...
void decreaseWater(long elapesdTime)
{
	for (int i = 0; i < 6; i++)
	{
		arrayFish[i] -= (level * 3 * (int)elapesdTime); // 3 : 난이도 조절을 위한 값
		if (arrayFish[i] < 0)
		{
			arrayFish[i] = 0;
		}
